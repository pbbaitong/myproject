...
def check_usr_role(tel_login: str):
    ...
    general = mycursor.execute("SELECT * FROM general WHERE general_tel = %s", (tel_login,))
    ...
    if general_existed is None:
        ...
        doctor = mycursor.execute("SELECT * FROM doctor WHERE doctor_tel = %s", (tel_login,))
...
def check_usr_pass(tel_login: str, password_login: str, role_sign_up) -> bool:
    ...
    if role_sign_up == 'คนทั่วไป':
        ...
        general_password = mycursor.execute("SELECT general_password FROM general WHERE general_tel = %s", (tel_login,))
        ...
        doctor_password = mycursor.execute("SELECT doctor_password FROM doctor WHERE doctor_tel = %s", (tel_login,))
...
def check_name(tel_login: str, role_sign_up):
    mycursor = conn.cursor(buffered=True)
    if role_sign_up == 'คนทั่วไป':
        general = mycursor.execute("SELECT general_firstname, general_lastname FROM general WHERE general_tel = %s", (tel_login,))
        name = mycursor.fetchone()
        firstname = name[0]
        lastname = name[1]
    else:
        doctor = mycursor.execute("SELECT doctor_firstname, doctor_lastname FROM doctor WHERE doctor_tel = %s", (tel_login,))
        name = mycursor.fetchone()
        firstname = name[0]
        lastname = name[1]
